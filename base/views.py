...
def suscribe(request):
    if request.method == 'POST':
        ...
        send_email(subject='Welcome to Allahu Nuru Ala Nuru. We are thrilled to have you onboard.', from_email='hello@allahunurualanuru.com', mail_adds=adds, message="As a subscriber, you’ll be the first to know about our latest updates, exclusive offers, and insightful content tailored just for you. Our mission is to bring you the best in innovative solutions, community events and a sense of spiritual purpose. We’re excited to share this journey with you. \n Feel free to explore our website, and don’t hesitate to reach out with any questions or feedback. We value your input and are here to help! \n\nThank you for joining us. Let’s make great things happen together!")
        prayer_times = PrayerTime.objects.all()
        events = Event.objects.order_by('start')
        main = events.first()
        events = events.exclude(id = main.id)
